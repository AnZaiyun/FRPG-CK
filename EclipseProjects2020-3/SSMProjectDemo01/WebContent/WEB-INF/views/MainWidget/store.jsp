...
<html>
  <head> 
    ...
    <script id="BuyItem">
		function BuyItem(sdid, itemid)
		{ 
			//使用Ajax，购买物品后无法更新剩余物品信息，采用直接跳转的方式
			var count=prompt("请输入要购买的物品数量","1");

			if (count!=null && count!="")
			{
				window.location.href = "/SSMProjectDemo01/MainForm/store?sdid="+sdid+"&itemid="+itemid+"&count="+count;
    ...
    <div class="d-flex align-items-stretch">
      ...
      <nav id="sidebar">
        ...
        <ul class="list-unstyled">
          ...
          <li class="active"><a href="/SSMProjectDemo01/MainForm/store"> <i class="icon-home"></i>Store </a></li>
            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="block margin-bottom-sm">
                  <div class="title"><strong>GoldStores</strong></div>
                  <div class="table-responsive"> 
                    <table class="table">
                      <thead>
                        <tr>
                          <th></th>
                          <c:forEach var="name" items = "${requestScope.FliedNames}">
                          <th>${name}</th>
                          </c:forEach>
                        </tr>
                      </thead>
                      <tbody>
                        <c:forEach var="item" items = "${requestScope.GoldStores}">
                        <tr>
                          <th scope="row"></th>
                          <c:if test="${item.sdid == 1}">
                          	  <td>${item.wpmc}</td>
	                          <td>${item.wplb}</td>
	                          <td>${item.price}</td>
	                          <td>${item.sl}</td>
	                          <td>${item.sm}</td>
	                          <td><a href = "#" onclick="BuyItem(1,${item.itemid})">购买</a></td>
                          </c:if>
                        </tr>
                        </c:forEach>
                      </tbody>
              ...
              <div class="col-lg-12">
                <div class="block margin-bottom-sm">
                  <div class="title"><strong>DiamondStores</strong></div>
                  <div class="table-responsive"> 
                    <table class="table">
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        <c:forEach var="item" items = "${requestScope.DiamondStores}">
                        <tr>
                          <th scope="row"></th>
                          <c:if test="${item.sdid == 2}">
                          	  <td>${item.wpmc}</td>
	                          <td>${item.wplb}</td>
	                          <td>${item.price}</td>
	                          <td>${item.sl}</td>
	                          <td>${item.sm}</td>
	                          <td><a href = "#" onclick="BuyItem(2,${item.itemid})">购买</a></td>
                          </c:if>                     
                        </tr>
                        </c:forEach>
                      </tbody>
